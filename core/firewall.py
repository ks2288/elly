...
def characteristic_blocked(service_uuid, characteristic_uuid, rule):
    ...
    if SERVICE_UUID in rule and CHARACTERISTIC_UUID in rule:
        if service_uuid.lower() == rule.get(SERVICE_UUID).lower() and characteristic_uuid.lower() == rule.get(
...
def descriptor_blocked(service_uuid, characteristic_uuid, descriptor_uuid, rule):
    ...
    if SERVICE_UUID in rule and CHARACTERISTIC_UUID in rule and DESCRIPTOR_UUID in rule:
        if service_uuid.lower() == rule.get(SERVICE_UUID).lower() and characteristic_uuid.lower() == rule.get(
                CHARACTERISTIC_UUID).lower() and descriptor_uuid.lower() == rule.get(DESCRIPTOR_UUID).lower():
            return True
    ...
    if SERVICE_UUID not in rule and CHARACTERISTIC_UUID in rule and DESCRIPTOR_UUID in rule:
        if characteristic_uuid.lower() == rule.get(CHARACTERISTIC_UUID).lower() and descriptor_uuid.lower() == rule.get(
                DESCRIPTOR_UUID).lower():
            return True
...
def descriptor_is_allowed(bdaddr, descriptor_handle):
    if not firewall_is_enabled():
        return True
    descriptor_uuid = gatt.get_descriptor_uuid(bdaddr, descriptor_handle)
    parent_uuids = gatt.get_owning_uuids(bdaddr, descriptor_handle)
...
def characteristic_is_allowed(bdaddr, characteristic_handle):
    if not firewall_is_enabled():
        return True
    characteristic_uuid = gatt.get_characteristic_uuid(bdaddr, characteristic_handle)
    service_uuid = gatt.get_owning_service_uuid(bdaddr, characteristic_handle)
...
def test1():
    devices_discovered = gap.discover_devices(3)
    print("DISCOVERED")
    print("----------")
...
def test2():
    ...
    gap.connect(bdaddr)
    services_discovered = gatt.get_services(bdaddr)
    print("DISCOVERED")
    print("----------")
...
def test3():
    ...
    for service in allowed_services:
        ...
        characteristics_discovered = gatt.get_characteristics(bdaddr, service['handle'])
        print()
...
def test4():
    ...
    for service in allowed_services:
        ...
        for characteristic in allowed_characteristics:
            ...
            descriptors_discovered = gatt.get_descriptors(bdaddr, characteristic['handle'])
            print()
            ...
            allowed_descriptors = filter_descriptors(bdaddr, service['UUID'], characteristic['UUID'],
                                                     descriptors_discovered)
            print()
...
def test5():
    ...
    print(
        "Access to characteristic " + gatt.get_characteristic_uuid(bdaddr, char_path) + " is allowed? " + str(
            allowed))
...
def test6():
    ...
    print("Access to descriptor " + gatt.get_descriptor_uuid(bdaddr, desc_path) + " is allowed? " + str(
        allowed))
